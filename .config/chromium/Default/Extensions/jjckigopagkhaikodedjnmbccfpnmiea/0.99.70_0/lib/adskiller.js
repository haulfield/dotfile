...
require.scopes["prefs"] = (function()
{
  ...
  defaults.subscriptions_exceptionsurl = "https://update.adskiller.me/whitelist.txt";
  defaults.subscriptions_antiblockurl = "";
  ...
  Downloader.prototype = {
    ...
    _download: function(downloadable, redirects)
    {
      ...
      var errorCallback = function errorCallback(error)
      {
        ...
        Cu.reportError("Ads Killer: Downloading URL " + downloadable.url + " failed (" + error + ")\n" + "Download address: " + downloadUrl + "\n" + "Channel status: " + channelStatus + "\n" + "Server response: " + responseStatus);
      ...
      try
      {
        ...
        if (if_modified_since[downloadable.url] && downloadable.lastVersion) {
          request.setRequestHeader("If-Modified-Since", if_modified_since[downloadable.url]);
        }
      }
      catch (e)
      {
      ...
      request.addEventListener("load", function(event)
      {
        ...
        setModifiedSince(downloadable.url, request.getResponseHeader("Last-Modified"));
...
require.scopes["filterStorage"] = (function()
{
  ...
  var FilterStorage = exports.FilterStorage = {
    ...
    _generateFilterData: function(subscriptions)
    {
      ...
      _generatorResult15.push("# Ads Killer preferences");
  ...
  var ElemHide = exports.ElemHide = {
    ...
    getSelectorsForDomain: function(domain, specificOnly)
    {
      ...
      for (var _loopIndex21 = 0; _loopIndex21 < keys.length; ++_loopIndex21)
      {
        ...
        {
          var new_domain = domain.replace("www.", "").toUpperCase();
          if (this.getException(filter, domain) && filter.domains && (new_domain in filter.domains)) {
              var white_filter = this.getException(filter, domain);
              whites.push(white_filter);
              continue;
          }
          result.push(filter.selector);
        }
      }
      return { selectors: result, whites: whites };
    }
  };
  return exports;
})();
...
require.scopes["filterListener"] = (function()
{
  ...
  var CSSPropertyFilter = _tempVar23.CSSPropertyFilter;
  var Prefs = require("prefs").Prefs;
  var batchMode = false;
  var isDirty = 0;
  var FilterListener = {
  ...
  var Synchronizer = exports.Synchronizer = {
    ...
    _onDownloadSuccess: function(downloadable, responseText, errorCallback, redirectCallback)
    {
      ...
      chrome.storage.local.get( 'customsublist', function(storage) {
          ...
          if (customsublist[subscription.url]) {
              if (params["title"]) {
                  customsublist[subscription.url]["title"] = params["title"];
...
require.scopes["antiadblockInit"] = (function()
{
  ...
  exports.initAntiAdblockNotification = function initAntiAdblockNotification()
  {
    ...
    {
      var subscription = Subscription.fromURL(Prefs.subscriptions_antiblockurl);
      if (subscription.url in FilterStorage.knownSubscriptions)
      {
    ...
    {
      if (!/^subscription\.(updated|removed|disabled)$/.test(action) || value.url != Prefs.subscriptions_antiblockurl)
      {
        return;
      }
...
require.scopes["url"] = (function()
{
  ...
  exports.extractHostFromUrl = function(url)
  {
...
require.scopes["stats"] = (function()
{
  ...
  FilterNotifier.addListener(function(action, item, newValue, oldValue, page)
  {
    ...
    {
      chrome.pageAction.setIcon({tabId: page._id, path: 'icons/icon19.png'}, callbackLastError);
...
require.scopes["icon"] = (function()
{
  ...
  {
    chrome.pageAction.setIcon({tabId: page._id, path: getIconPath(whitelistedState.get(page))}, callbackLastError);
  ...
  {
    chrome.pageAction.setIcon({tabId: page._id, path: getIconPath(whitelisted)}, callbackLastError);
...
require.scopes["notificationHelper"] = (function()
{
  ...
  var showNextNotification = exports.showNextNotification = function(url)
  {
    ...
    if (activeNotification.type == "critical" || activeNotification.type == "question")
    {
      ...
      var iconUrl = ext.getURL("icons/icon128.png");
...
require.scopes["devtools"] = (function()
{
  ...
  function matchRequest(request, white)
  {
    var text = "";
    if (white) {
      text = "@@";
    }
    var filter = text + generateFilter(request, false);
    filter = Filter.fromText(filter);
    filter.is_request_add = true;
    return filter;
  ...
  exports.logHiddenElements = function(page, selectors, docDomain, is)
  {
    var panel = getActivePanel(page);
    if (!panel) return;
    {
    ...
    for (var tabId in panels)
    {
      ...
      for (var i = 0; i < panel.records.length; i++)
      {
        ...
        if (added)
        {
          ...
          if (filters[0].text.indexOf("@@") == 0) {
            var filter = matchRequest(record.request, true);
          } else {
            var filter = matchRequest(record.request, false);
          }
          if (filters.indexOf(filter) == -1)
          {
            continue;
          }
          if (record.filter && !record.filter.is_request_add) {
            record.old_filter = record.filter;
          } else  {
            record.old_filter = undefined;
          }
        ...
        else
        {
          ...
          if (record.old_filter && !record.old_filter.is_request_add) {
            record.filter = record.old_filter;
          } else {
  ...
  function onFilterAdded(action, item, newValue, oldValue, page)
  {
    if (action != "filter.added")
    {
      return;
    }
    updateFilters([item], true);
  }

  function onFilterRemoved(action, item, newValue, oldValue, page)
  {
    if (action != "filter.removed")
    {
      return;
    }
    updateFilters([item], false);
  }
  ...
  chrome.runtime.onConnect.addListener(function(port)
  {
    ...
    if (!hasPanels())
    {
      ...
      FilterNotifier.addListener(onFilterAdded);
      FilterNotifier.addListener(onFilterRemoved);
    ...
    port.onDisconnect.addListener(function()
    {
      ...
      if (!hasPanels())
      {
        ...
        FilterNotifier.removeListener(onFilterAdded);
        FilterNotifier.removeListener(onFilterRemoved);
...
require.scopes["csp"] = (function()
{
  ...
  chrome.webRequest.onHeadersReceived.addListener(function(details)
  {
    ...
    var match = defaultMatcher.matchesAny("", "WEBSOCKET", hostname, false, null, true);
