...
CKGSClient.prototype.private_SendMessage = function(oMessage)
{
	...
	req.open("POST", "https://www.gokgs.com/json/access", true);
...
CKGSClient.prototype.private_ReceiveMessage = function()
{
	...
	req.open("GET", "https://www.gokgs.com/json/access", true);
	req.send();
};
...
CKGSClient.prototype.private_HandleLoginSuccess = function(oMessage)
{
	...
	if (Friends)
	{
		for (var Pos = 0, Count = Friends.length; Pos < Count; ++Pos)
		{
			...
			var oUserExt  = {
				User : oUser,
				Notes : oFriend["notes"] ? oFriend["notes"] : ""
			};

			if ("buddy" === oFriend["friendType"])
			{
				this.m_oFriendList[sUserName] = oUserExt;
			}
			else if ("censored" === oFriend["friendType"])
			{
				this.m_oBlackList[sUserName] = oUserExt;
			}
			else if ("fan" === oFriend["friendType"])
			{
				this.m_oFollowerList[sUserName] = oUserExt;
			}
		}
	}
...
CKGSClient.prototype.private_HandleGameRecord = function(oGameRecord, bAdd)
{
	...
	if (EKGSGameType.Challenge === nGameType)
	{
		...
		if (oCreator && oCreator.IsRobot())
			sWhite += "(" + g_oLocalization.mainRoom.gamesList.listAdditionalRobot + ")";
	}
...
CKGSClient.prototype.private_HandleUserUpdate = function(oMessage)
{
	...
	var oRoom = this.m_oPrivateChats[this.m_nChatChannelId];
	oRoom = oRoom ? oRoom : this.m_aAllRooms[this.m_nChatChannelId];
	if (oRoom && oRoom.Users && oRoom.Users[oUser.GetName()])
	{
		this.m_oPlayersListView.Handle_Record([0, oUser.GetName(), oUser.GetRank(), oUser.IsFriend(), oUser]);
		//this.m_oPlayersListView.Update_Size();
	}
...
CKGSClient.prototype.private_HandleFriendAddSuccess = function(oMessage)
{
	...
	var oUserExt    = {
		User  : oUser,
		Notes : oMessage["notes"] ? oMessage["notes"] : ""
	};
...
CKGSClient.prototype.private_UpdateRoomStats = function()
{
	var oRoom    = this.m_aAllRooms[this.m_nChatChannelId];
	var oPrivate = this.m_oPrivateChats[this.m_nChatChannelId];
	if (oRoom)
	{
		this.m_oApp.OnRoomStatsChanged(oRoom.UsersCount, oRoom.GamesCount, oRoom.ChallengesCount, 0);
	}
	else if (oPrivate)
	{
		var nUsersCount = 0;
		for (var sUserName in oPrivate.Users)
		{
			nUsersCount++;
		}
		this.m_oApp.OnRoomStatsChanged(nUsersCount, 0, 0, 0);
	}
	else
	{
		this.m_oApp.OnRoomStatsChanged(0, 0, 0, 0);
	}
};