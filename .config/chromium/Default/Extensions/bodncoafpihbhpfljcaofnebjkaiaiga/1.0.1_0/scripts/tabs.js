window.createTabs = function (config, utils, chrome) {
    ...
    var drawAttentionToPanel = function (tab, callback) {
        chrome.tabs.update(tab.id, {active: true}, function () {
            chrome.windows.update(tab.windowId, {drawAttention: true}, callback);
        });
    };
    ...
    var createNewPanel = function (roomName, callback) {
        chrome.windows.create({
            url: utils.chatRoomNameToUrl(roomName),
            type: 'panel',
            width: 400,
            height: 500
        }, callback);
    ...
    return {
        ...
        createOrActivatePanel: function (roomName, callback) {
            forSingleTabExtensionWindows(utils.chatRoomNameToUrl(roomName), function (windows) {
                if (windows.length < 1) {
                    createNewPanel(roomName, callback);
                } else {
        ...
        createOrDrawAttentionToPanel: function (roomName, callback) {
            forSingleTabExtensionWindows(utils.chatRoomNameToUrl(roomName), function (windows) {
                if (windows.length < 1) {
                    createNewPanel(roomName, callback);
                } else {
                    drawAttentionToPanel(windows[0].tabs[0], callback);
                }
            });
        },
